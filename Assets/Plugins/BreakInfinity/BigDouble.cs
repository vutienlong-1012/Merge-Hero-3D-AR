...
namespace BreakInfinity
{
    ...
    public struct BigDouble : IFormattable, IComparable, IComparable<BigDouble>, IEquatable<BigDouble>
    {
        ...
        static string[] smallNumberLetter = new string[] { "K", "M", "B", "T" };
        static string[] bigNumberLetter = new string[] { "a", "a", "a", "b", "b", "b", "c", "c", "c", "d", "d", "d", "e", "e", "e", "f", "f", "f", "g", "g", "g", "h", "h", "h", "i", "i", "i", "j", "j", "j", "k", "k", "k", "l", "l", "l", "m", "m", "m", "n", "n", "n", "o", "o", "o", "p", "p", "p", "q", "q", "q", "r", "r", "r", "s", "s", "s", "t", "t", "t", "u", "u", "u", "v", "v", "v", "w", "w", "w", "x", "x", "x", "y", "y", "y", "z", "z", "z" };

        public static string ToText(BigDouble value)
        {
            ...
            if (value.Exponent <= 14)
            {
                ...
                {
                    //value /= 1e3;
                    return value.ToDouble().ToString("0");//+ smallNumberLetter[0];
                }
                else
                if (value.Exponent >= 4 && value.Exponent <= 8)
                {
                    value /= 1e3;
                    return value.ToDouble().ToString("0.##") + smallNumberLetter[0];
                }
                ...
                /* Old Format
                if (value.Exponent >= 3 && value.Exponent <= 5)
                {
                    value /= 1e3;
                    return value.ToDouble().ToString("0.##") + smallNumberLetter[0];
                }
                else
                if (value.Exponent >= 6 && value.Exponent <= 8)
                {
                    value /= 1e6;
                    return value.ToDouble().ToString("0.##") + smallNumberLetter[1];
                }
                else*/

                if (value.Exponent >= 9 && value.Exponent <= 11)
                {
                    value /= 1e9;
                    return value.ToDouble().ToString("0.##") + smallNumberLetter[2];
                }
                else
                if (value.Exponent >= 12 && value.Exponent <= 14)
                {
                    value /= 1e12;
                    return value.ToDouble().ToString("0.##") + smallNumberLetter[3];
                }
                return value.ToDouble().ToString("0");
            }
            else
            {
                if (value.Exponent >= 15 && value.Exponent <= 2042)
                {
                    long tempa = (value.exponent - 15) % 78;
                    long tempb = ((value.exponent - 15) / 78) * 3;

                    value /= BigDouble.Parse("1e" + (value.exponent - (value.exponent % 3)).ToString());
                    return value.ToString("F2") + bigNumberLetter[tempb] + bigNumberLetter[tempa];
                }
                else
