namespace Abu
{
    ...
    public class TutorialFadeImage : Image
    {
        ...
        public void StartFadeInToHightLight()
        {
            this.DOFade(0.95f, 0.5f);
            Smoothness = 0.0002f;
            DOTween.To(() => Smoothness, x => Smoothness = x, 0.0003f, 0.5f).SetLoops(-1, LoopType.Yoyo);
        }

        public void StartFadeOutToHightLight(Action _onCompleteAction)
        {
            this.DOFade(0.0f, 0.5f).OnComplete(_onCompleteAction.Invoke);
            Smoothness = 0.0002f;
        }
    }
}
